...
from rag_pipelines_gemini import (
...
st.set_page_config(page_title="FEMA Emergency Planning Assistant (Gemini)")

st.title("⚡ FEMA Emergency Planning Assistant (Gemini-powered)")
st.markdown("Ask any FEMA-related question to compare how different RAG strategies respond, now powered by Gemini.")
...
if st.button("💬 Get Answer") and query:
    with st.spinner("🔄 Processing your query... Please wait."):
        try:
            ...
            # This will now call the Gemini-powered RAG functions
            ...
            if not os.path.exists("reference/fema_evaluation_gemini.csv"):
                st.warning("⚠️ CSV file not found at 'reference/fema_evaluation_gemini.csv'")
            else:
                update_results_csv(
                    question=query,
                    results=results,
                    csv_path="reference/fema_evaluation_gemini.csv",
                    trace_id=trace_id,
