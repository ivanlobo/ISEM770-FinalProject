...
def update_results_csv(question, results, csv_path="reference/fema_evaluation_hybrid.csv", trace_id=None, start_time=None, end_time=None):
    import pandas as pd
    import os
    ...
    try:
        # --- RAG responses ---
        df.at[idx, 'RAG Answer'] = results['hybrid'].get('response', "")

        # --- Similarity and BERT Scores ---
        val = results.get('validation', {})
        if 'hybrid' in val:
            df.at[idx, 'Similarity_Score'] = val['hybrid'].get('embedding', "")
            df.at[idx, 'Bert Score'] = val['hybrid'].get('bert_f1', "")
        else:
            df.at[idx, 'Similarity_Score'] = ""
            df.at[idx, 'Bert Score'] = ""
